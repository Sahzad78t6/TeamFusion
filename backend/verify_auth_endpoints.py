...
async def run_verification():
    ...
    db = get_database()
    if db is not None:
        await db[COLLECTION_USERS].delete_many({"email": "test.verified.user@growthos.ai"})
        print("--> Cleaned up existing test user record in MongoDB Atlas.")
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        res_short = await client.post("/api/auth/signup", json={
        ...
        res_refresh = await client.post("/api/auth/refresh", json={"refresh_token": refresh_token})
