...
export const Landing: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#090a0f] text-slate-900 dark:text-slate-100 selection:bg-purple-500 selection:text-white overflow-x-hidden font-sans relative">
      ...
      {/* Top Navbar */}
      <nav className="sticky top-0 z-40 flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-white/10 bg-white/80 dark:bg-[#090a0f]/80 backdrop-blur-xl max-w-7xl mx-auto rounded-b-2xl mt-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={toggleTheme}
            className="p-2.5 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors"
            title={`Switch to ${theme === 'dark' ? 'Light' : 'Dark'} mode`}
          >
            {theme === 'dark' ? <Sun className="w-4.5 h-4.5 text-amber-550 dark:text-amber-300" /> : <Moon className="w-4.5 h-4.5 text-indigo-650 dark:text-indigo-400" />}
          </button>
      ...
      <section className="relative pt-20 pb-24 px-6 max-w-6xl mx-auto text-center z-10">
        ...
        <h1 className="text-4xl sm:text-6xl md:text-7xl font-extrabold text-slate-900 dark:text-white tracking-tight leading-tight max-w-4xl mx-auto">
          We don't optimize for attention.{' '}
          <span className="text-gradient block mt-2">We optimize for human potential.</span>
        </h1>

        <p className="mt-6 text-lg sm:text-xl text-slate-600 dark:text-slate-400 max-w-2xl mx-auto font-normal h-8">
          {typedText}
        ...
        <motion.div
          ...
          className="mt-16 relative rounded-3xl border border-slate-200 dark:border-white/15 bg-white/70 dark:bg-slate-900/70 p-3 shadow-2xl shadow-slate-100 dark:shadow-purple-950/60 backdrop-blur-2xl overflow-hidden group hover:border-purple-500/40 transition-colors"
        >
          <div className="absolute top-0 left-0 right-0 h-10 bg-slate-100 dark:bg-white/5 border-b border-slate-200 dark:border-white/10 flex items-center px-4 gap-2">
            <div className="w-3 h-3 rounded-full bg-rose-500/80" />
            <div className="w-3 h-3 rounded-full bg-amber-500/80" />
            <div className="w-3 h-3 rounded-full bg-emerald-500/80" />
            <span className="mx-auto text-xs text-slate-500 dark:text-slate-400 font-mono">app.growthos.ai/dashboard</span>
          </div>
          ...
          <div className="pt-10 pb-4 px-4 grid grid-cols-1 md:grid-cols-3 gap-4 text-left">
            ...
            <TiltCard className="p-5 space-y-3">
              ...
              <p className="text-2xl font-extrabold text-slate-900 dark:text-white">Founder Archetype</p>
              <div className="w-full bg-slate-200 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
                <div className="bg-gradient-to-r from-purple-500 to-indigo-500 h-full w-[88%]" />
              </div>
              <span className="text-[11px] text-slate-500 dark:text-slate-400">Identity drift reduced by 12% this week.</span>
            </TiltCard>
            ...
            <TiltCard className="p-5 space-y-3">
              ...
              <p className="text-2xl font-extrabold text-slate-900 dark:text-white">92 / 100</p>
              <div className="flex items-center gap-2 text-xs text-slate-700 dark:text-slate-300">
                <Activity className="w-4 h-4 text-emerald-400" />
                <span>Burnout Risk: 14% (Low)</span>
              </div>
              <span className="text-[11px] text-slate-500 dark:text-slate-400">24 consecutive daily learning reflections.</span>
            </TiltCard>
            ...
            <TiltCard className="p-5 space-y-3">
              ...
              <p className="text-sm font-bold text-slate-900 dark:text-white line-clamp-1">LangGraph Multi-Agent Architecture</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">4.5h Course • DeepMind & GrowthOS</p>
      ...
      <section id="features" className="py-20 px-6 max-w-6xl mx-auto z-10 relative">
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold text-slate-900 dark:text-white mt-3">An Intelligent OS For Your Mind & Career</h2>
          <p className="text-slate-600 dark:text-slate-400 text-sm mt-3">GrowthOS merges multi-agent LLM logic, continuous memory vector pools, and machine learning models to curate your future.</p>
        </div>
      ...
      <section id="architecture" className="py-20 px-6 max-w-6xl mx-auto z-10 relative">
        <div className="glass-panel p-8 md:p-12 rounded-3xl border border-slate-200 dark:border-white/10 relative overflow-hidden">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="space-y-4 max-w-lg text-left">
              <Badge variant="blue">System Architecture</Badge>
              <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white">Built on Frontier Agentic Stack</h2>
              ...
              <div className="grid grid-cols-2 gap-3 text-xs font-semibold text-slate-700 dark:text-slate-300 pt-2">
                ...
                <div className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-emerald-400" /> MongoDB Database</div>
                <div className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-emerald-400" /> Scikit-Learn ML Models</div>
              </div>
            </div>

            <div className="w-full md:w-1/2 p-6 rounded-2xl bg-slate-50 dark:bg-black/60 border border-slate-200 dark:border-purple-500/30 font-mono text-xs text-slate-800 dark:text-slate-300 space-y-2 shadow-2xl shadow-slate-200/50 dark:shadow-purple-950/50">
              <div className="text-purple-600 dark:text-purple-400 font-bold">// LangGraph Workflow Graph</div>
              <div className="text-slate-500 dark:text-slate-400">User Prompt → Supervisor Router</div>
              <div className="pl-4 text-emerald-600 dark:text-emerald-400">├── Identity Agent (Evaluates Drift)</div>
              <div className="pl-4 text-indigo-600 dark:text-indigo-400">├── Learning Curator (Fetch Resources)</div>
              <div className="pl-4 text-cyan-600 dark:text-cyan-400">└── Memory Sync (Mem0 Vector Update)</div>
              <div className="text-amber-600 dark:text-amber-400 pt-2">Result: 98% Aligned Output Generated</div>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Footer */}
      <footer className="border-t border-slate-200 dark:border-white/10 py-12 px-6 max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6 text-xs text-slate-400 dark:text-slate-500 relative z-10">
        <div className="flex items-center gap-2">
          <Zap className="w-4 h-4 text-purple-500" />
          <span className="font-bold text-slate-800 dark:text-slate-300">GrowthOS © 2026</span>
        ...
        <div className="flex items-center gap-6">
          ...
          <a href="#" className="hover:text-slate-900 dark:hover:text-slate-300">GitHub</a>
          <a href="#" className="hover:text-slate-900 dark:hover:text-slate-300">Twitter / X</a>
        </div>
      </footer>
    </div>
  );
